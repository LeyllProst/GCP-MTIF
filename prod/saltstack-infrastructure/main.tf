...
# SALTMASTER instance
module "saltmaster" {
  source = "git@github.com:LeyllProst/gcp-mtif-instances.git?ref=b/2.0.2"

  project_id    = data.terraform_remote_state.vcp.outputs.project
  network       = data.terraform_remote_state.vcp.outputs.network_name
  subnetwork    = data.terraform_remote_state.vcp.outputs.subnetworks_name[1]
